 ...
 */
	public function resetPassword($postData = array()) {
		...
		$this->set($postData);
		if ($this->validates()) {
			$this->data[$this->alias]['password'] = $this->hash($this->data[$this->alias]['new_password'], 'blowfish', false);
 ...
 */
	public function validateOldPassword($password) {
        ...
        $oldPassword = crypt($password['old_password'], $currentPassword);
 ...
 */
	public function register($postData = array(), $options = array()) {
		...
		$this->set($postData);
		if ($this->validates()) {
			$postData[$this->alias]['password'] = $this->hash($postData[$this->alias]['password'], 'blowfish', false);
			$this->create();
